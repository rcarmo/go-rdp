...
func TestWrapTPKT(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:     "empty data",
			data:     []byte{},
			expected: []byte{3, 0, 0, 4}, // version, reserved, length (4 = header only)
		},
		{
			name:     "single byte",
			data:     []byte{0xAA},
			expected: []byte{3, 0, 0, 5, 0xAA}, // length = 5
		},
		{
			name:     "multiple bytes",
			data:     []byte{0x01, 0x02, 0x03},
			expected: []byte{3, 0, 0, 7, 0x01, 0x02, 0x03},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := WrapTPKT(tt.data)
			assert.Equal(t, tt.expected, result)
		})
	}
}

func TestUnwrapTPKT(t *testing.T) {
	tests := []struct {
		name        string
		input       []byte
		expectedLen int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			dataLen, err := UnwrapTPKT(reader)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
				assert.Equal(t, tt.expectedLen, dataLen)
			}
		})
	}
}

func TestUnwrapTPKT_EmptyReader(t *testing.T) {
	// Reader with no data
	reader := bytes.NewReader([]byte{})
	_, err := UnwrapTPKT(reader)
	assert.Error(t, err)
}

func TestTPKT_RoundTrip(t *testing.T) {
	original := []byte{0xDE, 0xAD, 0xBE, 0xEF}
	wrapped := WrapTPKT(original)

	reader := bytes.NewReader(wrapped)
	dataLen, err := UnwrapTPKT(reader)
	require.NoError(t, err)
	assert.Equal(t, len(original), dataLen)

	// Read the remaining data
	remaining := make([]byte, dataLen)
...
func TestWrapX224ConnectionRequestPDU(t *testing.T) {
	tests := []struct {
		name string
		data []byte
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := WrapX224ConnectionRequestPDU(tt.data)
			assert.Equal(t, byte(6+len(tt.data)), result[0]) // LI
			assert.Equal(t, byte(0xE0), result[1])           // CR + CDT
			assert.Equal(t, tt.data, result[7:])             // Variable part
		})
	}
}

func TestWrapX224DataPDU(t *testing.T) {
	data := []byte{0xAA, 0xBB}
	result := WrapX224DataPDU(data)

	assert.Equal(t, byte(0x02), result[0]) // packet size
	assert.Equal(t, byte(0xF0), result[1]) // message type TPDU_DATA
	assert.Equal(t, byte(0x80), result[2]) // EOT flag
	assert.Equal(t, data, result[3:])
}

func TestUnwrapX224ConnectionConfirmPDU(t *testing.T) {
	// Create a valid X224 Connection Confirm PDU
	// LI (1) + Fixed part (6) + Variable part
	data := []byte{
		6,          // LI (points to start of variable part)
		0xD0,       // CC + CDT
		0x00, 0x00, // DST-REF
		0x00, 0x00, // SRC-REF
		0x00,       // CLASS OPTION
		// Variable part (rdpNegData)
		0x01, 0x02, 0x03,
	}

	reader := bytes.NewReader(data)
	result, err := UnwrapX224ConnectionConfirmPDU(reader, len(data))
	require.NoError(t, err)
	assert.Equal(t, []byte{0x01, 0x02, 0x03}, result)
}

func TestUnwrapX224ConnectionConfirmPDU_EmptyReader(t *testing.T) {
	reader := bytes.NewReader([]byte{})
	_, err := UnwrapX224ConnectionConfirmPDU(reader, 10)
	assert.Error(t, err)
}

func TestUnwrapX224DataPDU(t *testing.T) {
	// LI (1) + Fixed part (2) + Data
	data := []byte{
		2,    // LI
		0xF0, // message type
		0x80, // EOT
	...
	reader := bytes.NewReader(data)
	result, err := UnwrapX224DataPDU(reader, len(data))
	require.NoError(t, err)
	assert.Equal(t, []byte{0xAA, 0xBB, 0xCC}, result)
}

func TestUnwrapX224DataPDU_EmptyReader(t *testing.T) {
	reader := bytes.NewReader([]byte{})
	_, err := UnwrapX224DataPDU(reader, 10)
	assert.Error(t, err)
}
...
func TestWrapSecurityFlag(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			flag:     0x0001,
			data:     []byte{0xAA, 0xBB},
			expected: []byte{0x01, 0x00, 0x00, 0x00, 0xAA, 0xBB},
		},
		{
			name:     "high flag value",
			flag:     0xFF00,
			data:     []byte{0x01},
			expected: []byte{0x00, 0xFF, 0x00, 0x00, 0x01},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := WrapSecurityFlag(tt.flag, tt.data)
			assert.Equal(t, tt.expected, result)
		})
	}
}
...
func TestUnwrapSecurityFlag(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "high flag value",
			input:        []byte{0x00, 0xFF, 0x00, 0x00},
			expectedFlag: 0xFF00,
			wantErr:      false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			reader := bytes.NewReader(tt.input)
			flag, err := UnwrapSecurityFlag(reader)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func TestUnwrapSecurityFlag_ReadError(t *testing.T) {
	// Not enough data for flags
	reader := bytes.NewReader([]byte{0x01})
	_, err := UnwrapSecurityFlag(reader)
	assert.Error(t, err)

	// Enough for flags but not for flagsHi
	reader = bytes.NewReader([]byte{0x01, 0x00})
	_, err = UnwrapSecurityFlag(reader)
	assert.Error(t, err)
}

func TestSecurityFlag_RoundTrip(t *testing.T) {
	originalFlag := uint16(0x1234)
	originalData := []byte{0xDE, 0xAD}

	wrapped := WrapSecurityFlag(originalFlag, originalData)
	reader := bytes.NewReader(wrapped)

	flag, err := UnwrapSecurityFlag(reader)
	require.NoError(t, err)
	assert.Equal(t, originalFlag, flag)

	// Read remaining data
	remaining := make([]byte, len(originalData))
	_, err = reader.Read(remaining)
	require.NoError(t, err)
	assert.Equal(t, originalData, remaining)
}

// =============================================================================
// MCS Domain PDU Tests
// =============================================================================

func TestWrapMCSDomainPDU(t *testing.T) {
	tests := []struct {
		name     string
		tag      uint8
		data     []byte
		...
		{
			name:     "tag 0 empty data",
			tag:      0,
			data:     []byte{},
			expected: []byte{0x00},
		},
		{
			name:     "tag 25 (send data)",
			tag:      25,
			data:     []byte{0xAA},
			expected: []byte{100, 0xAA}, // 25 << 2 = 100
		},
		{
			name:     "tag 1 with data",
			tag:      1,
			data:     []byte{0x01, 0x02},
			expected: []byte{4, 0x01, 0x02}, // 1 << 2 = 4
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := WrapMCSDomainPDU(tt.tag, tt.data)
			assert.Equal(t, tt.expected, result)
		})
	}
}

func TestUnwrapMCSDomainPDU(t *testing.T) {
	tests := []struct {
		name    string
		tag     uint8
		input   []byte
		...
		},
		{
			name:    "valid tag 25",
			tag:     25,
			input:   []byte{100}, // 25 << 2 = 100
			wantErr: false,
		},
		{
			name:    "mismatched tag",
			tag:     10,
			input:   []byte{100}, // 25 << 2, but expecting 10
			wantErr: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			reader := bytes.NewReader(tt.input)
			err := UnwrapMCSDomainPDU(tt.tag, reader)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func TestUnwrapMCSDomainPDU_ReadError(t *testing.T) {
	reader := bytes.NewReader([]byte{})
	err := UnwrapMCSDomainPDU(0, reader)
	assert.Error(t, err)
}

func TestWrapMCSSendData(t *testing.T) {
	data := []byte{0x01, 0x02, 0x03}
	result := WrapMCSSendData(data)

	// Should be equivalent to WrapMCSDomainPDU(25, data)
	expected := WrapMCSDomainPDU(25, data)
	assert.Equal(t, expected, result)
	assert.Equal(t, byte(100), result[0]) // 25 << 2
}

func TestMCSDomainPDU_RoundTrip(t *testing.T) {
	tag := uint8(25)
	data := []byte{0xAA, 0xBB}

	wrapped := WrapMCSDomainPDU(tag, data)
	reader := bytes.NewReader(wrapped)

	err := UnwrapMCSDomainPDU(tag, reader)
	require.NoError(t, err)
